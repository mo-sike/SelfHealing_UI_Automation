...
# Force UTF-8 stdout so Unicode chars don't crash on Windows cmd
...
COLOR_CHANGE_THRESH = 0.05

# Minimum IOU to count as a position-based match (Pass 3)
IOU_MATCH_THRESH = 0.40

# Text-change detection: if two matched elements have non-empty, differing text
# longer than this, classify as color_change (same position/size, different content).
# Covers label renames and content updates visible to users.
MIN_TEXT_LEN_FOR_TEXT_CHANGE = 6
...
def classify_change(e1, e2, img1=None, img2=None, verbose=False):
    """
    Classify the type of change between two matched elements.

    Returns: (change_type str or None, debug_dict)
        change_type — "relocate" | "resize" | "color_change" | None
        debug_dict  — {dx, dy, dw, dh, pdiff, text_diff} for verbose output
    """
    b1 = e1["bounds"]
    b2 = e2["bounds"]
    ...
    t1 = e1.get("text", "").strip()
    t2 = e2.get("text", "").strip()
    text_changed = (
        len(t1) >= MIN_TEXT_LEN_FOR_TEXT_CHANGE and
        len(t2) >= MIN_TEXT_LEN_FOR_TEXT_CHANGE and
        t1 != t2
    )

    dbg = {"dx": round(dx,1), "dy": round(dy,1),
           "dw": round(dw,3), "dh": round(dh,3),
           "pdiff": round(pdiff,3), "text_diff": text_changed}

    if moved and resized:
        ct = "relocate" if max(dx, dy) > max(dw, dh) * min(w1, h1) else "resize"
        return ct, dbg
    ...
    if pdiff > COLOR_CHANGE_THRESH:
        return "color_change", dbg

    # Text content changed at same position/size → visible label/content change
    if text_changed:
        return "color_change", dbg
...
def annotate_pair(pair_id, img1_path, img2_path, dump1_path, dump2_path,
                  output_dir, yolo_model=None, dry_run=False, verbose=False):
    """
    ...
    Args:
        ...
        verbose    : print per-element debug info (why each pair was skipped)
    ...
    if verbose:
        print(f"  [DBG] {pair_id}: img={w}×{h}  "
              f"elems1={len(elems1)}  elems2={len(elems2)}")
    ...
    if verbose:
        print(f"  [DBG] {pair_id}: matched={len(matched)}  "
    ...
    for e1, e2 in matched:
        ct, dbg = classify_change(e1, e2, img1, img2, verbose=verbose)
        if ct is None:
            skipped_none += 1
            if verbose:
                rid = e1.get("resource_id") or e1.get("content_desc") or e1.get("text","")[:20]
                print(f"    [no-change] {rid!r:30s}  "
                      f"dx={dbg['dx']:5.1f} dy={dbg['dy']:5.1f}  "
                      f"dw={dbg['dw']:.3f} dh={dbg['dh']:.3f}  "
                      f"pdiff={dbg['pdiff']:.3f}  text_diff={dbg['text_diff']}")
            continue
    ...
    if verbose and skipped_none:
        print(f"  [DBG] {pair_id}: {skipped_none} matched pairs had no detectable change")
...
def annotate_pairs_dir(pairs_dir, output_dir=None, yolo_model=None,
    ...
    Use --verbose to see per-element debug output explaining zero-annotation pairs.
    """
...
def annotate_all_apps(root_dir, yolo_model=None, dry_run=False, verbose=False):
    """
    ...
    for app_dir in sorted(app_dirs):
        ...
        print(f"\n═══ {app_dir.name} ══════════════════════════════════════")
        results = annotate_pairs_dir(pairs_dir, pairs_dir, yolo_model, dry_run, verbose)
...
def main():
    ...
    parser.add_argument(
        ...
        help="Print per-element debug info for every pair (useful to diagnose 0-annotation pairs)"
    )
    ...
    # ── Annotate ─────────────────────────────────────────────────────────────
    if args.root_dir:
        results = annotate_all_apps(args.root_dir, yolo_model, args.dry_run, args.verbose)
    else:
        out = args.output_dir or args.pairs_dir
        results = annotate_pairs_dir(args.pairs_dir, out, yolo_model,
