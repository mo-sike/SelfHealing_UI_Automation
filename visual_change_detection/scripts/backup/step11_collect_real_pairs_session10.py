...
def download_fdroid_apk(package_name, version_info, output_dir):
    ...
    if not apk_name:
        print(f"[WARN] No apkName in version info: {version_info}")
        return None
...
def download_app_apks(app_cfg, output_dir):
    """
    Download two APK versions for an app from F-Droid.
    Returns (v1_apk_path, v2_apk_path) or (None, None) on failure.
    """
    package = app_cfg["fdroid_package"]
...
def find_apk_for_app(app_cfg, apk_dir, version_label):
    """
    Look for a pre-downloaded APK for an app in apk_dir.
    Naming convention: {AppName}_v1.apk / {AppName}_v2.apk
    Returns path string or None.
    """
    app_name = app_cfg["name"].replace(" ", "")
    candidate = Path(apk_dir) / f"{app_name}_{version_label}.apk"
    if candidate.exists():
        return str(candidate)
    # Try fuzzy match by package name fragment in filename
    for f in Path(apk_dir).glob("*.apk"):
        pkg_frag = app_cfg["package"].split(".")[-1].lower()
        if pkg_frag in f.name.lower() and version_label in f.name.lower():
            return str(f)
    return None
...
def main():
    ...
    if not device_id:
        ...
        if len(devs) == 1:
            ...
            sys.exit(1)
        elif not (args.download or args.download_all):
