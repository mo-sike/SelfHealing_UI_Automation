...
def triage(heal_result, change_type, confidence,
    ...
    Verdict table:
        AUTO-HEAL    — element unchanged (sim ≥ threshold) AND pixel-diff
                       confirms no visual change (change_type == "no_change"),
                       OR change in [relocate, resize] AND confidence ≥ 0.80
    ...
    Note: when status == "unchanged" but the pixel-diff sanity check found
    visual dissimilarity (pxd > UNCHANGED_PIXEL_DIFF_THRESH), the classifier
    overrides change_type from "no_change" to the detected change type.
    In that case we fall through to confidence-based triage so false greedy
    matches are not silently auto-healed.
    ...
    # Unchanged — locator still valid as-is ONLY when pixel-diff also confirms
    # no visual change.  If the pixel-diff check overrode change_type (e.g. to
    # "relocate"), fall through to classifier-driven triage below.
    if status == "unchanged" and change_type in ("no_change", None):
