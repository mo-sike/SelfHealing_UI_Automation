...
def main():
    ...
    if args.collect:
        if not args.app:
            print("[ERROR] --app is required with --collect")
            sys.exit(1)
        registry = load_registry(args.registry)
        ...
        # Auto-detect APKs from --apk_dir when --v1_apk / --v2_apk not given
        v1_apk = args.v1_apk
        v2_apk = args.v2_apk
        if not v1_apk or not v2_apk:
            v1_apk = find_apk_for_app(app_cfg, args.apk_dir, "v1")
            v2_apk = find_apk_for_app(app_cfg, args.apk_dir, "v2")
        if not v1_apk or not v2_apk:
            print(f"[ERROR] Could not find APKs for {args.app}. "
                  f"Pass --v1_apk / --v2_apk explicitly, or run --download_all first.")
            sys.exit(1)
